...
用法：
    python scripts/schedule.py "data/輸入/你的.xlsx"

它會：
  1. 執行完整自動排班（日/時/室/老師）。
  2. 把結果檔 <你的檔名>_排班結果.xlsx 儲存到 data/輸出/ 資料夾，
...
def run(xlsx_path: str) -> None:
    ...
    out_dir = os.path.join(_PKG_ROOT, "data", "輸出")
    os.makedirs(out_dir, exist_ok=True)
    stem = os.path.splitext(os.path.basename(xlsx_path))[0]
    out_path = os.path.join(out_dir, f"{stem}_排班結果.xlsx")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print('用法：python scripts/schedule.py "data/輸入/你的.xlsx"')
        sys.exit(1)
    run(sys.argv[1])
